<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="com.kh.snackking.contract.model.vo.Contract, java.util.*"%>
			...
			<div id="searchArea">
				...
				<div id="subSubTitle1">계약 정보</div>
					...
					<!-- 등록버튼 삭제함 -->
						...
						<input type="hidden" name="userNo" value="<%=c.getUserNo()%>">
							<table class="memberTable">
							<tr>
								<td>거래처명 :</td>
								<!-- 여기 view 페이지에 있는 name=""(값 넘어가는것)하고 다음에 넘어가는 controller의 servlet getParameter("")하고 같아야 함!!! -->
								<td><input type="text" class="searchTextBox" size="15" name="corpName" value="<%=c.getCorpName()%>"> 
								</td>
								<td></td>
								<td></td>
								<td></td>
							...
							<tr>
								<td>사업자등록번호:</td>
								<td><input type="text" class="searchTextBox" size="15" name="businessNo" value="<%=c.getBusinessNo()%>">
								</td>
								<td style="text-align:center">계약일 :</td>
								<td><input type="date" class="searchTextBox" size="10" name="conDate" value="<%=c.getConDate()%>"></td>
								<td></td>
								<td></td>
							</tr>
							<tr>
								<td>계약기간 :</td>
								<td colspan="2">
									<input type="date" class="searchTextBox" size="" name="startDate" value="<%=c.getStartDate()%>">&nbsp;&nbsp;&nbsp;~&nbsp;&nbsp; 
									<input type="date" class="searchTextBox" name="endDate" value="<%=c.getEndDate()%>"></td>
								<td></td>
								<td></td>
								<td></td>
							</tr>
							<tr>
								<td>배송횟수 :</td>
								<td>월 <input type="text" class="searchTextBox" size="10" name="delivCount" value="<%=c.getDelivCount()%>">회</td>
								<td style="text-align:center">회차당 금액 :</td>
								<td><input type="text" class="searchTextBox" size="10" name="amountPDeliv" value="<%=c.getAmountPDeliv()%>">원</td>
								<td>월 계약금액 :</td>
								<td><input type="text" class="searchTextBox" size="10" name="ttlAmount" value="<%=c.getTtlAmount()%>">원</td>
							</tr>
