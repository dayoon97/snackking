    ...
    var srcCalendar = new FullCalendar.Calendar(srcCalendarEl, {
      ...
      defaultView: 'dayGridMonth',
	      events: [
	    	  ...
	    	  <%for(int j=0; j<slist.get(i).getDelivCount(); j++) {%>
	    	  
	    	  {
	      	title : '<%=slist.get(i).getUserName()%>    '+'<%=j+1%>회차',
	      	start : '<%=slist.get(i).getScheduleDate()%>',
	      	id : '<%=slist.get(i).getUserNo() %>'
	    	  },
      ...
      eventClick: function(info){
    	...
    	/* $("#")// 모달 띄우기  */
    	<%for(int i=0; i<slist.size(); i++){%>
    		if(rId==<%=slist.get(i).getUserNo()%>){
    			$("#hiddenCompany").text("<%=slist.get(i).getCompany()%>");
    			$("#hiddenUName").text("<%=slist.get(i).getUserName()%>");
    			$("#hiddenCName").text("<%=slist.get(i).getMngId()%>");
    			$("#hiddenDCount").text("<%=slist.get(i).getDelivCount()%>");
    			$("#hiddenDTime").text("<%=slist.get(i).getScheduleDate()%>");
      ...
      eventDrop: function(info) {
        ...
        $("[name=changeInputDate]").val(info.event.start.toISOString().slice(0,10));
        var rId = info.event.id;
        
        $(".infoChange").css("display","block");
    	<%for(int i=0; i<slist.size(); i++){%>
	  		if(rId==<%=slist.get(i).getUserNo()%>){
		       	$("[name=changeInputDate]").val(info.event.start.toISOString().slice(0,10));	// 변경할 날짜
		       	$("#hiddenRNo").val(rId);
	  		}
	  	<%}%>
	...
	<div id="outer">
		...
		<div id='source-calendar'></div>
	      ...
	      <div class="infoModal-content">
	                <p style="text-align: center;"><span style="font-size: 14pt;"><b><span style="font-size: 24pt;">배송정보조회</span></b></span></p>
	                <table align="center">
	                	<tr>
	                		<td>상호명 : </td>
	                		<td><p id="hiddenCompany"></p></td>
	                		<td>고객명 : </td>
	                		<td><p id="hiddenUName"></p></td>
	                		<!-- <td>담당큐레이터 : </td>
	                		<td><p id="hiddenCName"></p><td> -->
	                	</tr>
	                	<tr>
	                		<td></td>
	                		<td><p id="hiddenRName"></p></td>
	                		<td></td>
	                		<td><p id="hiddenRPrice"></p></td>
	                		
	                	</tr>
	                	<tr>
	                		<td>월배송회차 : </td>
	                		<td><p id="hiddenDCount"></p></td>
	                		<td>배송시간 : </td>
	                		<td><p id="hiddenDTime"></p></td>
	                		
	                	</tr>

	                </table>
	            <div style="cursor:pointer;background-color:#DDDDDD;text-align: center;padding-bottom: 10px;padding-top: 10px;">
	                <form method="post" action="<%=request.getContextPath()%>/delete.ca">
	                	<button type="button" onclick="deleteReservation();" style="font-size: 13pt;">삭제</button>
	                </form>
	                <button type="button" class="pop_bt" onClick="close_pop();" style="font-size: 13pt;">닫기</button>
	            </div>
	      </div>
	     </div>
		...
		<div id="changePop" class="infoChange">
	      ...
	      <div class="infoChange-content">
	                <p style="text-align: center;"><span style="font-size: 14pt;"><b><span style="font-size: 24pt;">배송일정변경</span></b></span></p>
	                <form>
	                	<table align="center">
	                		<tr>
		                		<td>현재 날짜</td>
			                ...
			                <tr>
			                	<td><input type="hidden" id="hiddenRNo" name="hiddenRNo"></td>
			                </tr>
	                	</table>
			            <div style="cursor:pointer;background-color:#DDDDDD;text-align: center;padding-bottom: 10px;padding-top: 10px;">
			                	<button type="button" onclick="changeRinfo();" style="font-size: 13pt;">변경</button>
			                <button type="button" class="pop_bt" onClick="window.location.reload();" style="font-size: 13pt;">닫기</button>
			            </div>
	                </form>
	      </div>
	     </div>
  		...
  		<script>
  			...
  			function goHome(){
  				location.href="<%=request.getContextPath()%>/views/calen.jsp";
  			}
  		</script>
		<script>
